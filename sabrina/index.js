...
///✅ INFORMAÇÕES ✅
const { forwarding, crtt, visualizarmsg, dono1, dono2, dono3, dono4, dono5, dono6 } = require("./settings/nescessario.json");
const { fundo1, fundo2, thumbnail } = require("./settings/links_img.json");
const { NomeDoBot, NickDono, prefix, API_KEY_SABRINA, TOKEN_CONSULTAS } = require("./settings/settings.json");
const { audio_menu, bom_dia, boa_tarde, boa_noite, corno, qviado } = require('./settings/media/audios.json');
const { enviar } = require('./settings/message.js');
...
.replace("÷", "/");
if (!rsp.match(/^[\d+\-*/.() ]+$/)) {
return reply('❗ Erro: A expressão fornecida é inválida. Tente novamente.');
}
console.log(
'[', color('CALCULAR', 'silver'), ']',
color(moment(info.messageTimestamp * 1000).format('DD/MM HH:mm:ss'), 'yellow'),
color(rsp)
);
try {
const resultado = new Function('return ' + rsp)();
return reply(`O resultado de ${rsp} é: ${resultado}`);
} catch (error) {
console.log('[', color('ERROR', 'red'), ']', error);
return reply('Erro: Não foi possível calcular a expressão.');
}
break;


case 'cnpj':
let cnpj = body.slice(6).trim();
if (!cnpj) return reply('❗ Uso incorreto! Use: !cnpj <CNPJ>');
if (!/^\d{14}$/.test(cnpj)) return reply('❗ CNPJ inválido! É nescessário enviar o CNPJ sem nenhum espaço ou símbolo.');
let cnpjUrl = `https://www.receitaws.com.br/v1/cnpj/${encodeURIComponent(cnpj)}`;
axios.get(cnpjUrl)
.then(response => {
...
let cep = body.slice(5).trim();
if (!cep) return reply('❗ Uso incorreto! Use: !cep <CEP>');
if (!/^\d{5}-?\d{3}$/.test(cep)) return reply('❗ CEP inválido!');
let cepUrl = `https://viacep.com.br/ws/${encodeURIComponent(cep)}/json/`;
axios.get(cepUrl)
.then(response => {
let data = response.data;
if (data.erro) {
...
let infoCep = `📍 *Informações do CEP:*\n\n\n🔹 *CEP:* ${data.cep}\n\n🔹 *Logradouro:* ${data.logradouro}\n\n🔹 *Bairro:* ${data.bairro}\n\n🔹 *Cidade:* ${data.localidade}\n\n🔹 *Estado:* ${data.uf}`;
reply(infoCep);
}
})
.catch(error => {
console.error(error);
reply('❗ Ocorreu um erro ao buscar o CEP. Por favor, tente novamente mais tarde.');
});
break;
//PARA GRUPOS 👥
...
case 'ping': {
  ...
  // Ajustar o caminho pro armazenamento compartilhado
  const discoInfo = execSync('df -h ~/storage/shared', { encoding: 'utf-8' }).split('\n')[1];
  const [ , tamanhoTotal, usado, livre ] = discoInfo.split(/\s+/);
...
case 'welcome': {
  ...
  if (!isGroupAdmins) return reply('Apenas administradores podem usar esse comando!');
  if (!isBotGroupAdmins) return reply('Eu preciso ser administrador para executar esse comando!');

  const participant = args[0]; // Número do novo membro
  if (!participant) return reply('Informe o número do participante!');

  const groupMetadata = await conn.groupMetadata(from); // Dados do grupo

  try {
    // Verificar se o usuário tem foto de perfil
    const profilePictureUrl = await conn.profilePictureUrl(participant, 'image').catch(() => null);

    let message = `👋 Olá @${participant.split('@')[0]}, seja bem-vindo(a) ao grupo: *${groupMetadata.subject}*`;
    let options = { mentions: [participant] };

    if (profilePictureUrl) {
      // Baixar a foto de perfil
      const filePath = `./temp/${participant.split('@')[0]}-profile.jpg`;
      const imageBuffer = await downloadProfilePicture(profilePictureUrl);
      fs.writeFileSync(filePath, imageBuffer);

      // Enviar mensagem com a foto
      await conn.sendMessage(from, { caption: message, image: { url: filePath } }, options);

      // Apagar a foto após enviar
      fs.unlinkSync(filePath);
    } else {
      // Enviar só a mensagem, se não tiver foto
      await conn.sendMessage(from, { text: message }, options);
    }
  } catch (err) {
    console.error('Erro ao enviar mensagem de boas-vindas:', err);
    reply('Não consegui enviar a mensagem de boas-vindas.');
  }
  break;
}
